...
void OSDAnalog::decode(const TMSPSimulatorFromINAV* message)
{

  if (message->osdInfo.analog.osdRow == 255)
  {
    return; //no OSD data
  }

  this->auto_rows = (message->osdInfo.analog.osdRow & 0x80) ? 16 : 13;

  int osdRow = message->osdInfo.analog.osdRow & 0x7f;
  int osdCol = message->osdInfo.analog.osdCol;
...
void OSDAnalog::disconnect(void)
{
}
...
//==============================================================
//==============================================================
void OSDAnalog::drawChar(uint16_t code, float x1, float y1, float width, float height)
{
  int code9 = code % 0xff;
...
void OSDAnalog::clear()
{
  memset(this->osdData, 0, OSD_ROWS * OSD_COLS * 2);
}

void OSDAnalog::draw(TOSDType osd_type, bool smoothed)
{
  ...
  */
  ...
  if (osd_type == OSD_NTSC)
  {
    rowsCount = OSD_ROWS_NTSC;
  }
  else if (osd_type == OSD_AUTO)
  {
...
void OSDAnalog::setFont(std::string fontName)
{
 // Not supported
}

std::string OSDAnalog::getFont()
{
  return std::string();
}

void OSDAnalog::destroy(void)
{
  destroyTexture(this->fontTextureId);
}

uint16_t OSDAnalog::getChar(int row, int col)
{
  if (row >= OSD_ROWS || col >= OSD_COLS)
    return 0;

  return CHAR_BYTE(this->osdData[row * OSD_ROWS + col]);
}

void OSDAnalog::setChar(int row, int col, char c)
{
  this->osdData[OSD_COLS * row + col] = MAKE_CHAR_MODE(c, 0);
}

int OSDAnalog::getColCount()
{
  return OSD_COLS;
}

int OSDAnalog::getRowCount()
{
  return OSD_ROWS;
}
